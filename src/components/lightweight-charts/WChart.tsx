...
// Union of every series API produced by createSeries. Using a single
// ISeriesApi<any> keeps the helper functions (which call generic series
// methods) type-checking under the strict tsconfig while preserving behavior.
...
const WChart: React.FC<WChartProps> = ({
	...
	useEffect(() => {
		...
		const shouldShowEma =
			showEma && !isMiniChart && chartType === "candlestick";
		...
		chart.timeScale().applyOptions({ fixLeftEdge: true, rightOffset: timeScaleRightOffset });
...
function toVolumeData(item: ChartDataItem): HistogramData & { color: string } {
	const close = item.close ?? item.value ?? 0;
	const open = item.open ?? item.value ?? 0;
	return {
...
function calcEMA(data: ChartDataItem[], period: number): LineData[] {
	...
	const result: LineData[] = [
		{ time: data[period - 1].time as Time, value: ema },
	];
